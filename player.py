...
class TransformerPlayerV3_2(Player):
    ...
    def __init__(self, name: str = "TransformerPlayerV3_2"):
        super().__init__(name)
        self._model        = None
    ...
    def _load_model(self):
        try:
            ...
            print(f"[TransformerPlayerV3_2] Loading {self.HF_MODEL_ID} on {self._device}...")
            ...
            self._model.eval()
            print(f"[TransformerPlayerV3_2] Ready on {self._device}")
        except Exception as e:
            print(f"[TransformerPlayerV3_2] Load failed: {e}. Falling back to CPU.")
            self._device = "cpu"
            self._model = None
    ...
    def _heuristic_score(self, board: chess.Board, move: chess.Move) -> float:
        ...
        score -= repeat_count * 15.0  # 权重增加，让 AI 打破循环
    ...
    def _heuristic_top_n(self, board: chess.Board, legal_moves: list, n: int = 15):
    ...
    def _score_legal_moves(self, fen: str, legal_uci: list) -> str:
        ...
        top_moves_uci = self._heuristic_top_n(board, [chess.Move.from_uci(m) for m in legal_uci])
        ...
        for move_uci in top_moves_uci:
            ...
            h_score = self._heuristic_score(board, move_obj)
